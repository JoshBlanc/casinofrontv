...
const ProteinaList = ({ proteinas, onAdd, onEdit, onDelete }) => {
  return (
    <CCard className="shadow-sm border-0 rounded-4">
      ...
      <CCardHeader className="d-flex justify-content-between align-items-center bg-white border-0">
        <div>
          ...
          <small className="text-medium-emphasis">Gestión de proteínas registradas</small>
        </div>
            ...
            {proteinas.length > 0 ? (
              proteinas.map((p) => (
                <CTableRow key={p.id}>
                  ...
                  <CTableDataCell>
                    <CButton size="sm" color="warning" className="me-2" onClick={() => onEdit(p)}>
                      Editar
                    </CButton>

                    <CButton size="sm" color="danger" onClick={() => onDelete(p.id)}>
                      Eliminar
                    </CButton>
                  </CTableDataCell>
                </CTableRow>
